            ...
            <section className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain pr-1">
              {notifications.map((note) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    onNotificationOpen?.(note.id);
                    navigate(`/profile?tab=orders&order=${encodeURIComponent(note.orderId)}`);
                    onClose?.();
                  }}
                  className="w-full rounded-2xl border border-slate-100 bg-white px-4 py-4 text-left shadow-md shadow-slate-200/60 transition hover:shadow-lg"
                  aria-label={`Open details for order ${note.orderNumber}`}
                >
